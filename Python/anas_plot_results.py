...
def plot_oscillator_patterns(times, outputs, drive, vlines=[0,20,40], walk_timesteps=[16, 17], swim_timesteps=[23, 24]):
    ...
    axes[3].axhline(y=5, xmin=0, xmax=3, color="orange", linewidth=1, zorder=0)
    axes[3].axhline(y=2.5, xmin=0, xmax=3, color="orange", linewidth=1, zorder=0)
...
def plot_properties(times, outputs, drive, freqs_log, amplitudes_log, vlines=[0,20,40], walk_timesteps=[16, 17], swim_timesteps=[26, 27]):
    ...
    # Assign single label for body and limb
    label = None
    axes[0].plot(times, outputs[:, 0], color='black', label=label)
    axes[0].plot(times, outputs[:, 16]-2,color='black', linestyle='dashed', label=label)
    # axes[0].text(-1.5, outputs[0, 0], f'$x$', fontsize='small')
    # Remove the y-ticks
    axes[0].yaxis.set_tick_params(labelleft=False)
    axes[0].set_yticks([])
    # Add trunk and tail labels
    ...
    # Plot both oscillators
    axes[1].plot(times, freqs_log[:, 0], label=label, color='black')
    axes[1].plot(times, freqs_log[:, 16], label=label, color='black', linestyle='dashed')
    # Equal axis size as previous
    #axes[1].axis('equal')
    
    axes[2].set_ylabel('r')
    # Amplitudes
    axes[2].plot(times, amplitudes_log[:, 0], label=label,  color='black')
    axes[2].plot(times, amplitudes_log[:, 16], label=label, color='black', linestyle='dashed')
    # Equal axis size as previous
    #axes[2].axis('equal')
    ...
    axes[3].set_ylabel('drive d')
    axes[3].plot(times, drive, 'black')
    axes[3].axhline(y=5, xmin=0, xmax=3, color="black", linestyle='dotted', linewidth=1, zorder=0)
    axes[3].axhline(y=2.5, xmin=0, xmax=3, color="black", linestyle='dotted', linewidth=1, zorder=0)
